...
class CommunicationMonitor:
    ...
    def display(self):
        ...
        # Get the elapsed times for each service
        step_request_latency_time = self.get_elapsed_time(
            self._step_request_sent_timestamp, self._step_request_received_timestamp)
        step_request_process_time = self.get_elapsed_time(
            self._step_request_received_timestamp, self._step_response_sent_timestamp)
        step_response_latency_time = self.get_elapsed_time(
            self._step_response_sent_timestamp, self._step_response_received_timestamp)
        ...
        # Get the 'inference_time'
        inference_time = self.get_elapsed_time(
            self._previous_step_response_received_timestamp, self._step_request_sent_timestamp)
        ...
        step_request_latency_stats = calculate_statistics(self.step_request_latency_times)
        step_request_process_stats = calculate_statistics(self.step_request_process_times)
        step_response_latency_stats = calculate_statistics(self.step_response_latency_times)
        step_total_elapsed_stats = calculate_statistics(self.step_total_elapsed_times)
        inference_stats = calculate_statistics(self.inference_times)
...
{header}
{separator}
{format_row('Step request latency time [s]', step_request_latency_time, step_request_latency_stats)}
{format_row('Step request process time [s]', step_request_process_time, step_request_process_stats)}
{format_row('Step response latency time [s]', step_response_latency_time, step_response_latency_stats)}
{format_row('Total step elapsed time [s]', step_total_elapsed_time, step_total_elapsed_stats)}
{format_row('Inference time [s]', inference_time, inference_stats)}
"""
